...
function update_graph() {
  if (cpu_graph_draw_context) {
    background_page_window.draw_cpu_graph(background_page_window.cpu_history[current_graph], cpu_graph_draw_context, 600, 100, 0, 2, 1)
  }
}
...
function displayProcessInfo(process) {
  ...
  if (process.length) {
    ...
    if (process.type == 'renderer') {
      process.type = process.tasks[0].title;
    }
    if (process.type != 'browser') {
      process.type += ' [#' + process.id + ']';
    }
      // process.type =  process.tasks && process.tasks[0] && process.tasks[0].title;
  }
  var row_class = process.cpu > 15 ? 'warning' : '';
  if (process.id == current_graph) {
    row_class += ' selected';
  }
  row_html += '<tr class="' + row_class + '" data-tabid="' + (process.length ? tab[0] : '') + '" data-pid="' + process.id + '">' +
    '<td class="cpu">' + Math.floor(process.cpu) + '%</td>' +
    '<td class="memory">' + memory + '</td>' +
    '<td class="process">' + ucfirst(process.type) + '<span id="tabTitle' + tab + '"></span></td>' +
    '<td class="actions">';
  if (process.type == 'Tab') {
